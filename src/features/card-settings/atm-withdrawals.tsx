...
export function AtmWithdrawals() {
  ...
  const isEnabled = selectedCard?.security?.withdrawalEnabled
  ...
  const handleOnClick = async (value: boolean): Promise<void> => {
    ...
    try {
      await CardService.updateSecurity({
        ...
        // @ts-ignore
        securityData: { ...selectedCard.security, withdrawalEnabled: value },
      })
  ...
  return (
    <SettingItemCardWrap title={'ATM Withdrawals'} description={'Allow/Deny ATM withdrawals'}>
