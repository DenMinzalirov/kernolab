...
export function ContactlessPayment() {
  ...
  const isEnabled = selectedCard?.security?.contactlessEnabled
  const handleOnClick = async (value: boolean): Promise<void> => {
    ...
    try {
      await CardService.updateSecurity({
        ...
        // @ts-ignore
        securityData: { ...selectedCard.security, contactlessEnabled: value },
      })
  ...
  return (
    <SettingItemCardWrap title={'Contactless Payment'} description={'Turn on/off contactless payments'}>
